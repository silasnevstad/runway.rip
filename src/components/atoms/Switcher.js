...
const Switcher = ({
    ...
    activeBgColor = 'bg-bg-200 dark:bg-bg-900',
    inactiveBgColor = 'bg-bg-100 dark:bg-bg-800',
        ...
        <div className={`flex items-center rounded-full p-1 gap-1 ${border ? 'border' : ''} ${inactiveBgColor} ${className}`} {...props}>
            {options.map((option, index) => (
                <Tooltip key={index} text={tooltip && option.name} position="top">
                    <button
                        key={index}
                        className={`whitespace-nowrap flex items-center gap-2 py-2.5 px-5 text-center font-medium rounded-full ${vertical ? 'flex-col' : 'flex-row'} ${hover && 'hover:bg-bg-100 dark:hover:bg-bg-800'} ${border && 'border'} ${selected === option.value ? activeBorderColor : inactiveBorderColor} ${selected === option.value ? activeBgColor : inactiveBgColor} ${selected === option.value ? activeColor : inactiveColor}`}
