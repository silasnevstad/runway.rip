...
4. Copy the price ID (`price_1N...`) into [`config.js`](/docs/tutorials/config#payments).
    ...
    <Tip>
        In [`config.js`](/docs/tutorials/config#payments), I have provided a sample configuration for both one-time and recurring payments, select the one that fits your product.
        You can simply have one plan if you want.
    </Tip>

5. Tweak the [`pricingConfig`](/docs/tutorials/config#payments) object in [`config.js`](/docs/tutorials/config) to match your product.
For more info checkout [pricing config documentation](/docs/tutorials/config#payments).
...
<Callout>
    The [`Pricing`](/docs/landing/pricing) component will automatically pull everything it needs from the [`pricingConfig`](/docs/tutorials/config#payments) object.
</Callout>
