---
title: "WithWithout"
description: "A component to promote your product over competitors"
onThisPage:
  - title: "Overview"
    href: "#overview"
  - title: "Usage"
    href: "#usage"
  - title: "Props"
    href: "#props"
related:
  - title: "Card"
    href: "/docs/components/card"
---

<div id="overview" />

<br />

<div className="flex flex-col w-full items-center justify-center gap-8">
    <WithWithout />
</div>

<br />

The `WithWithout` component is a simple yet effective way to promote your product over competitors. It highlights the benefits of using your product while subtly pointing out the drawbacks of alternatives.

<br />
<br />
<br />
<Divider />
<div id="usage" />

## Usage

```js
import WithWithout from '@/components/WithWithout';

<WithWithout />
```

<br />
<br />
<br />
<Divider />

<div id="props" />

## Props

| Name             | Type    | Default                                                  | Description                                                                                                                  |
|------------------|---------|----------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------|
| title            | string  | `"Tired of spending hours configuring and maintaining?"` | The title of the component.                                                                                                  |
| withoutTitle     | string  | `"Without Runway"`                                       | The title of the component when the `without` prop is true.                                                                  |
| withTitle        | string  | `"With Runway"`                                          | The title of the component when the `without` prop is false.                                                                 |
| withouts         | array   | `withWithoutsConfig.withouts`                            | The list of items to display when the `without` prop is true, see [config](/docs/features/config#landing-page) for more details.  |
| withs            | array   | `withWithoutsConfig.see`                                 | The list of items to display when the `without` prop is false, see [config](/docs/features/config#landing-page) for more details. |
| cardBorderRadius | string  | `"lg"`                                                   | The border radius of the card (using [Tailwind](https://tailwindcss.com) border radius classes).                             |
| cardBorder       | boolean | `false`                                                  | If true, a border will be displayed on the card.                                                                             |