...
Need payments or subscriptions? Lets set up Stripe ⬇︎
...
<Tip>
    You can use your own email or setup your domain email following the [Domain](/docs/tutorials/domain) tutorial.
</Tip>
...
4. Copy the price ID (`price_1N...`) into `src/config.js`.
    ...
    <Tip>
        In `src/config.js`, I have provided a sample configuration for both one-time and recurring payments, select the one that fits your product.
        You can simply have one plan if you want.
    </Tip>

5. Optionally, tweak the `pricingConfig` object in `src/config.js` to match your product (used for [`Pricing`](/docs/landing/pricing) component).
If you are going for a simple flow, you can skip this step and only change the price IDs.
    ...
    <Callout>
        The [`Pricing`](/docs/landing/pricing) component will automatically pull everything it needs from the `pricingConfig` object.
    </Callout>
...
Now we need to set up webhooks to listen for events from Stripe.
...
4. Paste this URL as the endpoint URL (replace `YOURDOMAIN.COM` with your domain):
    ...
    <Tip>
        Don't have a domain? Check out [Domain](/docs/tutorials/domain) tutorial.
    </Tip>
...
<Caution>
    Make sure you test everything Test mode before going to Live mode. You can use the test card number 4242 4242 4242 4242 for a successful payment.
</Caution>
