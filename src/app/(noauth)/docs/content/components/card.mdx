---
title: "Card"
description: "A versatile card component with animation and hover effects"
onThisPage:
  - title: "Card"
    href: "#card"
  - title: "Props"
    href: "#props"
  - title: "Usage Examples"
    href: "#usage-examples"
---

<div id="card" />

<Divider />

<br/>

<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    <Card>
        <h2>Card Title</h2>
        <p>This is a basic card with default settings.</p>
    </Card>
    <Card hoverEffect="outline">
        <h2>Outlined Card</h2>
        <p>This card has an outline effect on hover.</p>
    </Card>
    <Card hoverEffect="scale" className="bg-bg-200 dark:bg-bg-800">
        <h2>Scalable Card</h2>
        <p>This card scales up on hover.</p>
    </Card>
    <Card className="bg-blue-200 dark:bg-blue-800 text-blue-800 dark:text-blue-200">
        <h2>Custom Styled Card</h2>
        <p>This card has custom styles.</p>
    </Card>
</div>

<br />

The `Card` component is a versatile card component that can be used to display content in a structured and visually appealing way. It comes with animation and hover effects that can be customized to fit your design needs.

<Tip>
    Use the `hoverEffect` prop to add interactive elements to your cards without additional styling.
</Tip>

<br/>
<Divider />
<br/>

<div id="props" />

## Props

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| className | string | - | Additional CSS classes to be applied |
| children | ReactNode | - | The content to be displayed inside the card |
| hoverEffect | 'none', 'shadow-sm', 'scale', 'outline' | 'shadow-sm' | The hover effect to apply to the card |
| border | boolean | true | Whether to display a border around the card |

<Info>
    The component also accepts any additional props that can be applied to a div element.
</Info>

<br/>
<br/>
<Divider />
<br/>

<div id="usage-examples" />

## Usage Examples

### Basic Card
<PreviewCode preview={
    <Card>
        <h2>Card Title</h2>
        <p>This is a basic card with default settings.</p>
    </Card>
}>
```javascript
<Card>
    <h2>Card Title</h2>
    <p>This is a basic card with default settings.</p>
</Card>
```
</PreviewCode>

### Card with Hover Outline Effect
<PreviewCode preview={
    <Card hoverEffect="outline">
        <h2>Outlined Card</h2>
        <p>This card has an outline effect on hover.</p>
    </Card>
}>
```javascript
<Card hoverEffect="outline">
    <h2>Outlined Card</h2>
    <p>This card has an outline effect on hover.</p>
</Card>
```
</PreviewCode>

### Card with No Animation
<PreviewCode preview={
    <Card hoverEffect="none">
        <h2>Static Card</h2>
        <p>This card does not animate on mount.</p>
    </Card>
}>
```javascript
<Card hoverEffect="none">
  <h2>Static Card</h2>
  <p>This card does not animate on mount.</p>
</Card>
```
</PreviewCode>

### Card with Custom Styling
<PreviewCode preview={
    <Card className="bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-200" border={false}>
        <h2>Custom Styled Card</h2>
        <p>This card has custom background and text colors.</p>
    </Card>
}>
```javascript
<Card className="bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-200" border={false}>
  <h2>Custom Styled Card</h2>
  <p>This card has custom background and text colors.</p>
</Card>
```
</PreviewCode>

<Tip>
    Experiment with different combinations of props to create cards that fit your design needs.
</Tip>

<br/>
<br/>